...
switch ($_method) {
    ...
    case 'PATCH':
        ...
        break;
    case 'OPTIONS':
        if ($_authorization === $_token_options) {
            include_once '../conexion.php';
            include_once 'modelo.php';
            //se realiza la instancia al modelo Indicador
            $modelo = new Menu();
            $respuesta = $modelo->reset();
            if ($respuesta) {
                http_response_code(200);
                echo json_encode(['success' => 'Reseteado completo']);
            } else {
                http_response_code(403);
                echo json_encode(['error' => 'No se logro resetear']);
            }
        } else {
            http_response_code(403);
            echo json_encode(['error' => 'Prohibido']);
